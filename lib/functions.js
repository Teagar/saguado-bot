...
const wait = async (media) => new Promise(async (resolve, reject) => {
    ...
    if (!response.ok) reject(`Não Consegui Encontrar!`);
    const result = await response.json()
    try {
    	const { is_adult, title, title_chinese, title_romaji, title_english, episode, season, similarity, filename, at, tokenthumb, anilist_id } = result.docs[0]
    	let belief = () => similarity < 0.89 ? "Acho que o que você procura é isso : " : ""
    	let ecch = () => is_adult ? "Tem ecchi" : "Não tem ecchi"
...
const banner = cfonts.render(('SAGUADO|BOT'), {
    font: 'block',
    color: 'candy',
    align: 'center',
    gradient: ["red","yellow"],
